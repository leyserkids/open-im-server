...
// GetConversationReadCursors gets the read cursors (read seqs) for all group members in the given conversations.
// This is used for group message read receipt feature.
func (c *conversationServer) GetConversationReadCursors(ctx context.Context, req *pbconversation.GetConversationReadCursorsReq) (*pbconversation.GetConversationReadCursorsResp, error) {
	...
	for _, conversationID := range req.ConversationIDs {
		// Only process group conversations (sg_ or g_ prefix)
		...
		userIDs, err := c.groupClient.GetGroupMemberUserIDs(ctx, groupID)
		if err != nil {
			log.ZWarn(ctx, "GetGroupMemberUserIDs failed", err, "groupID", groupID)
			continue
		}
		...
		// Get read seqs for all users in this conversation
		...
		// Build cursor list
		cursors := make([]*pbconversation.GroupReadCursor, 0, len(userReadSeqs))
		for userID, readSeq := range userReadSeqs {
			cursors = append(cursors, &pbconversation.GroupReadCursor{
				UserID:     userID,
				MaxReadSeq: readSeq,
			})
		}

		resp.ConversationReadCursors = append(resp.ConversationReadCursors, &pbconversation.ConversationReadCursors{
